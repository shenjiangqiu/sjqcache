...
namespace sjq {
    ...
    public:
        ...
        void set_entry(unsigned long long tag, cache_entry_status status, int from_type) {
            m_tag = tag;
            m_status = status;
            m_type = from_type;
        }
    ...
    public:
        ...
        cache(int way, int set, rep_policy p, int mshr_num, int mshr_maxmerge, std::string name);
        ...
        cache::access_ret access(unsigned long long addr, access_type type, int from_type);
        ...
        cache_entry get_last_evict() const {
